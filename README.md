...
Список собственных прокси теперь можно **проверять на работоспособность прямо в
расширении**:

- **Многопоточная проверка.** Прокси проверяются параллельно (в несколько
  потоков), а не по одной, поэтому большой список обрабатывается заметно
  быстрее. Рядом с каждой записью отображается статус — **рабочий** (с
  задержкой в мс) или **не работает**.
- **Кнопки «Проверить все», «Остановить» и «Удалить нерабочие».** Проверку
  можно прервать в любой момент отдельной кнопкой; нерабочие прокси удаляются
  либо вручную одной кнопкой, либо **сразу по ходу проверки** (отдельная
  галочка) — не нужно ждать завершения всего списка.
- **Трафик не пропадает во время проверки.** Раньше при проверке активная
  прокся отключалась и трафик шёл напрямую. Теперь всё ваше обычное
  проксирование **продолжает работать через включённую проксю**, пока идёт
  проверка кандидатов.
- **Готовые подписки с регулярно обновляемыми проксями.** В разделе «Импорт
  прокси» можно одним кликом загрузить актуальный список из проверенных
  публичных источников (monosans — с валидацией, обновляется ежечасно; Proxifly
  — каждые ~5 минут; TheSpeedX — ежедневно) или указать **собственную ссылку**
  на список. Опция «Загружать списки через активный прокси» позволяет получить
  подписку, даже если её адрес заблокирован. После импорта список
  **сразу перерисовывается** — переоткрывать страницу не нужно.
- **Сворачиваемые списки.** Блоки «Импорт прокси» и «Мои прокси» можно свернуть,
