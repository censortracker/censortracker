...
`isPrivateHost`, `isIgnoredHost`, `matchBlockedSuffix` (в `host-rules.js`) и
`hashHost` (в `pac.js`) встраиваются в PAC как текст. Из этого следует жёсткое
ограничение:
...
4. **«Обрезать хост до второго уровня» — не то же самое, что «домен сайта».**
   `FindProxyForURL` раньше отрезал от хоста последние две метки и сравнивал
   результат с блок-листом на точное равенство. Свойство «одна запись покрывает
   все поддомены» это давало, но ценой двух молчаливых провалов: в списке лежит
   `example.co.uk` (`tldts` знает публичные суффиксы), а хост `www.example.co.uk`
   превращался в `co.uk` — совпадения нет **никогда**, и так для всего под
   `.co.uk`, `.com.br`, `.org.uk` и прочими многосоставными суффиксами; а запись
   глубже двух меток (`cdn.example.com` в импортированном списке) не совпадала
   ни с чем (20.15.0).

   Сейчас список сверяется обходом суффиксов (`matchBlockedSuffix`), как и
   список игнорируемых. Поддомены по-прежнему покрываются одной записью, и все
   хосты одного заблокированного сайта по-прежнему идут через одну и ту же
   прокси — хэш берётся от **совпавшей записи**, а не от хоста, иначе долгие
   сессии рвались бы при переходе между поддоменами. Обход останавливается,
   не доходя до голого публичного суффикса: запись `com` могла появиться только
   из испорченного списка, а уважить её значило бы завернуть в проксю весь
   интернет.

   Ключ для страновых правил при этом остался прежним — `secondLevel(host)`.
   У него та же слабость с многосоставными суффиксами (правило для
   `example.co.uk` пишется под ключ `co.uk`), просто она пока никого не
   кусала: правила задаёт человек и для одного сайта.
...
9. **Минимальная версия браузера живёт в трёх местах и разъезжается.**
   ...
   На момент 20.15.0 ни один используемый API не требует больше Firefox 91:
...
10. **`proxyDNS` в `ProxyInfo` — по умолчанию `false`, и это утечка DNS.**
    ...
    которого обычная SOCKS4-прокси не понимает (20.15.0).
