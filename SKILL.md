...
Есть и вторая, менее очевидная зависимость: **цель транспиляции в `.babelrc`**.
Чем ниже целевая версия браузера, тем больше babel переписывает — и если он
решит вставить во встраиваемую функцию свой helper (`_slicedToArray`,
`_createForOfIteratorHelper` и подобные из `@babel/plugin-transform-runtime`),
то в PAC уедет ссылка на имя, которого там нет. Поэтому во встраиваемых функциях
не используются `for…of`, деструктуризация, spread и шаблонные строки: обычного
цикла по индексу babel не трогает ни при какой цели. **Меняя цель транспиляции,
перепроверяйте PAC, полученный из минифицированной сборки** — это единственное
место, где такая поломка видна.
...
9. **Минимальная версия браузера живёт в трёх местах и разъезжается.**
   `strict_min_version` / `minimum_chrome_version` в манифестах — это обещание
   магазина пользователю; `targets` в `.babelrc` — то, что сборка на самом деле
   умеет; README — то, что читают люди. Манифест обещал Firefox 91.1, а сборка
   транспилировалась под 98: пользователи 91–97 получали обновление, которое
   могло не запуститься. Поднимать минимум в манифесте, чтобы «сошлось», —
   худший из выходов: это отрезает существующие установки от обновлений. Сначала
   выясните, что код действительно требует (какие API и какие неполифилленные
   встроенные методы — `@babel/plugin-transform-runtime` без `corejs`
   полифиллит только helper'ы, но **не** `crypto.randomUUID`, `Object.hasOwn`
   и подобное), и подтяните к этому сборку и README.

   На момент 20.14.4 ни один используемый API не требует больше Firefox 91:
   `proxy.settings` и `proxy.onRequest` — с 60, `proxy.onError` — с 68, а
   `crypto.randomUUID`, `browser.scripting` и `browser.action` закрыты
   проверками с запасным путём.

10. **`proxyDNS` в `ProxyInfo` — по умолчанию `false`, и это утечка DNS.**
    Свойство легко привязать к «прокси с логином», потому что оно стоит рядом с
    `username`/`password`. Но слушатель `proxy.onRequest` подключается, как
    только SOCKS-логин нужен **хотя бы одному** узлу цепочки, и дальше отвечает
    за все — так узел без логина уходил с `proxyDNS` по умолчанию и резолвил
    имена локально, хотя та же самая прокся резолвила их удалённо, когда
    маршрутизацией управлял PAC. Ставьте `proxyDNS` каждому SOCKS5-хопу
    независимо от учётных данных. SOCKS4 — намеренное исключение: он не умеет
    передавать имя хоста, и запрос имени превращает соединение в SOCKS4a,
    которого обычная SOCKS4-прокси не понимает.

    Ставьте себе тот же вопрос про любое поле `ProxyInfo`: оно про
    аутентификацию или про маршрут? Про маршрут — значит нужно всем.
...
8. Если тронули `.babelrc` или минимальную версию в манифесте — прогнан ли PAC
   из минифицированной сборки? См. §1 и грабли №9.
