(function() {
  ...
  const checkDomains = (currentHostname, callbacks) => {
    let onMatchFoundCallback = callbacks['onMatchFound'];
    let onMatchNotFoundCallback = callbacks['onMatchNotFound'];
    ...
    db.getItem(DB_DOMAINS_ITEM_NAME)
      .then((data) => {
        ...
        let matchFound = domains.find(function(domain) {
  ...
  const checkDistributors = (hostname, callbacks) => {
    ...
    db.getItem(DB_DISTRIBUTORS_ITEM_NAME)
      .then((distributors) => {
        ...
        let domains = distributors.domains;
        let cooperationRefused = false;

        let matchFound = domains.find(function(item) {
  ...
  const reportBlockedByDPI = (domain) => {
    chrome.storage.local.get(
      ...
      (data) => {
        let alreadyReported = data.alreadyReported;
